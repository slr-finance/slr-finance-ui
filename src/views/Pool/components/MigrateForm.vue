<template>
  <div v-if="poolId === stakerState.poolId">
    ...
    >
      Migrate to {{ nextPool.name }} pool with {{ nextPoolApyStr }} APY
      <ui-icon
        name="arrow-right"
        class="ml-8"
      />
...
import { useStakingMigrate } from '../hooks/useStakingMigrate'
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const [handleMigrate, migrateTxState] = useStakingMigrate()
    const [handleUnstake, unstakeTxState] = useUnstake(poolId)

    const refetchStakerAndBalance = () => Promise.all([
      refetchStaker(),
      refetchSlrBalance(),
    ])
