...
import { computedEager, createSharedComposable, useIntervalFn } from '@vueuse/core'
...
export const usePresale = createSharedComposable(() => {
  ...
  const calls: Call[] = methods.map(({ name }) => ({
  ...
  useIntervalFn(() => {
    currentPhaseCountdown.value = Math.max(0, currentPhaseCountdown.value)
  })

  watch([currentPhaseCountdown], () => {
    console.log(currentPhaseCountdown.value)
